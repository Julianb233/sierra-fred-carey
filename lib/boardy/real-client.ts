...
export class RealBoardyClient implements BoardyClientInterface {
   ...
   * If no matches exist, triggers AI-generated suggestions as a fallback.
   */
  ...
  async refreshMatches(userId: string): Promise<BoardyMatch[]> {
    ...
    try {
        ...
        if (data.matches && Array.isArray(data.matches)) {
          ...
          for (const m of data.matches) {
            const match = await createMatch(supabase, {
              userId,
              matchType: mapMatchType(m.type),
              matchName: formatMatchName(m),
      ...
      logger.log(
        `[RealBoardy] API fetch failed, falling back: ${err instanceof Error ? err.message : String(err)}`
      );
    }
   ...
   */
  async requestCall(
    request: BoardyCallRequest
  ): Promise<BoardyCallResponse> {
    try {
      ...
      if (!response.ok) {
        ...
        logger.log(
          `[RealBoardy] Call request failed: ${response.status} ${errorText}`
        );
        return {
          success: false,
      ...
      logger.log(
        `[RealBoardy] Call requested for ${request.name || "unknown"}`
      );
  ...
  private async generateFallbackMatches(
