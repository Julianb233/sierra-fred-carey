...
async function logDecisionToMemory(
  ...
  try {
    const { logDecision } = await import("@/lib/db/fred-memory");
...
function buildAutoExecuteResponse(
  ...
  // Fast-path: content already generated by buildSimpleDecision — reuse it
  if (decision.metadata?.fastPath) {
    return {
      ...base,
      content: decision.content,
      action: decision.action,
...
async function updateConversationState(
  ...
  if (!conversationState) {
    console.warn('[FRED Execute] Skipping post-response state updates — conversationState not loaded');
    return;
  }
