...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Uses resilient logJourneyEvent with retry logic
    // Fetches previous score for delta tracking (non-blocking)
    (async () => {
      try {
        const scoreBefore = await getPreviousScore(userId, "analysis_completed");
        ...
        console.error("[Reality Lens API] Journey event save failed:", err);
      }
    })();
