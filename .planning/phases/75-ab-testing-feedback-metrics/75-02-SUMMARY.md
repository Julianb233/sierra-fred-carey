...
- File: `lib/feedback/pre-registration.ts`

### Task 2: Migration + API routes
- Migration adds `metadata JSONB DEFAULT '{}'` column to `ab_experiments`
- GET `/api/admin/ab-tests` now returns experiments with feedback metrics and pre-registration data
...
- AB tests page rewritten with ExperimentFeedbackCard
...
## Commit
- `cf90ece` — feat(75-02): pre-registration workflow and feedback-aware admin dashboard
