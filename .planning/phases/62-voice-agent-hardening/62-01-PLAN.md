...
<objective>
Harden the voice call client UI and fix remaining MEDIUM-severity bugs across webhook and worker.

Purpose: Address reconnection UX gap (users see no feedback during network hiccups) and fix 3 bugs identified in voice audit reports (metadata overwrite, fragile agent identity, unhandled worker errors).

Output: Updated call-fred-modal.tsx with reconnection UI, fixed webhook handler, and robust worker error handling.
</objective>
...
<task type="auto">
  <name>Task 2: Fix webhook metadata merge, agent identity detection, and worker error handling</name>
  <files>
    app/api/livekit/webhook/route.ts
    workers/voice-agent/agent.ts
  </files>
  <action>
Fix 3 MEDIUM-severity audit findings:
...
**C. Worker error handling (workers/voice-agent/agent.ts)**
...
Keep everything after waitForParticipant unchanged (encoder, publishTranscript, STT/LLM/TTS creation, session setup, shutdown callback, session.start, delay, greeting).
  ...
  <verify>
Run `npx tsc --noEmit` to confirm no type errors across both files. Verify the webhook metadata field now includes spread of existing metadata. Verify extractUserIdFromIdentity checks for exact 'fred-cary-voice' string. Verify agent.ts has try/catch around connect and waitForParticipant.
  </verify>
  <done>
Three MEDIUM-severity audit findings fixed: metadata merge preserves room_started fields, agent identity detection is exact-match, and worker handles connection failures gracefully. All files pass type checking.
  </done>
</task>
...
5. `agent.ts` has try/catch around `ctx.connect()` and `ctx.waitForParticipant()`
</verification>
