...
<objective>
Implement the Growth Agent -- the specialist agent for growth strategy and experimentation -- plus the agent tasks API endpoint and the agent dashboard UI. This completes the three-agent virtual team and provides users a way to view their agents.

Purpose: The Growth Agent helps founders with data-driven growth decisions. The dashboard UI makes the virtual team tangible and accessible.

Output: Growth agent with 4 domain tools, tasks API, agent dashboard page with status cards and task history.
</objective>
...
<task type="auto">
  <name>Task 2: Agent dashboard UI and tasks API</name>
  <files>
    app/api/agents/tasks/route.ts
    components/agents/agent-card.tsx
    components/agents/agent-task-list.tsx
...
- Loading skeleton state while fetching
  ...
  <verify>Page renders at /dashboard/agents. Agent cards display for all 3 agent types. Task list shows agent history. Non-Studio users see locked state. `npm run build` succeeds.</verify>
  <done>Agent dashboard shows 3 agent cards and task history. Studio tier gating enforced at UI level. Tasks API returns recent agent tasks.</done>
</task>
