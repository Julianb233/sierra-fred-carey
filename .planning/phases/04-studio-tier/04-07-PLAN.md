...
must_haves:
  ...
  artifacts:
    ...
    - path: "app/api/sms/preferences/route.ts"
      provides: "GET/POST API for SMS preferences management"
      exports: ["GET", "POST"]
  key_links:
    ...
    - from: "app/dashboard/sms/page.tsx"
      to: "app/api/sms/preferences/route.ts"
      via: "reads/writes SMS preferences via API"
      pattern: "fetch.*api/sms/preferences"
    - from: "app/api/sms/preferences/route.ts"
      to: "lib/db/sms.ts"
      via: "reads/writes user_sms_preferences via DB operations"
      pattern: "getUserSMSPreferences|updateSMSPreferences"
---
...
Output: Updated Stripe config, upgrade card, SMS settings page with phone number configuration and check-in history, and SMS preferences API.
</objective>
...
<task type="auto">
  <name>Task 2: Upgrade card, SMS settings UI, and SMS preferences API</name>
  <files>
    components/tier/upgrade-card.tsx
    components/sms/checkin-settings.tsx
    app/dashboard/sms/page.tsx
...
**app/api/sms/preferences/route.ts:**
GET handler - returns SMS preferences for authenticated user:
1. requireAuth() to get userId
2. Call getUserSMSPreferences(userId) from lib/db/sms.ts
3. Return 200 with { preferences } (or empty default if null)

POST handler - updates SMS preferences:
1. requireAuth() to get userId
2. Validate request body with Zod: { phoneNumber?: string, checkinEnabled?: boolean, checkinDay?: number (0-6), checkinHour?: number (0-23), timezone?: string }
3. Call updateSMSPreferences(userId, validatedBody) from lib/db/sms.ts
4. Return 200 with updated preferences

Export `const dynamic = 'force-dynamic'`

**app/dashboard/sms/page.tsx:**
"use client" page:
1. Fetch SMS preferences from `/api/sms/preferences` on mount
2. Show CheckinSettings component with current preferences
3. Wire onSave to POST `/api/sms/preferences` and refetch on success
4. Below settings, show check-in history: fetch from a GET call to `/api/agents/tasks` filtered or create a separate endpoint; simplest approach is to add a `getCheckinHistory` call inline. Fetch check-in history from the sms_checkins data.
...
7. Page title: "SMS Check-ins" with subtitle "Weekly accountability to keep you on track"
  ...
  <verify>Upgrade card renders for Pro users with correct pricing. SMS settings page shows preferences form and check-in history. SMS preferences API returns and saves data correctly. Tier gating works. `npm run build` succeeds.</verify>
  <done>Studio upgrade flow works via Stripe checkout. SMS settings allow phone configuration, schedule customization, and opt-in consent. SMS preferences API supports GET/POST. Check-in history shows timeline of messages.</done>
</task>
...
- app/api/sms/preferences/route.ts handles GET and POST for SMS preferences
...
- SMS preferences API correctly reads and writes user preferences
