...
files_modified:
  ...
  - package.json
  - app/api/pitch-deck/upload/route.ts
  - app/api/pitch-deck/parse/route.ts
...
must_haves:
  ...
  key_links:
    ...
    - from: "middleware.ts"
      to: "lib/api/cors.ts"
      via: "import corsHeaders, isWebhookPath"
      pattern: "corsHeaders"
---
...
**Step 6: Apply CORS headers via root middleware (PROD-06)**

Update the root middleware.ts created in Step 2 to apply CORS headers on all API responses. Add CORS handling BEFORE the auth check:
...
This ensures CORS headers are applied to ALL API routes automatically via middleware, rather than requiring each route to manually call the utility. The CORS utility functions remain available for any route that needs custom CORS behavior.

Also update the existing pitch-deck routes to remove their manual CORS handling since middleware now handles it:
- `app/api/pitch-deck/upload/route.ts` -- remove the OPTIONS handler and manual Access-Control headers
- `app/api/pitch-deck/parse/route.ts` -- remove the OPTIONS handler and manual Access-Control headers

**Step 7: Verify PROD-04 (Image optimization)**
...
4. **CORS utility exports and middleware wiring:**
   ```bash
   grep 'export function' lib/api/cors.ts
   # Verify CORS is applied in middleware
   grep 'corsHeaders\|isWebhookPath' middleware.ts
   # Verify pitch-deck routes no longer have manual CORS
   grep -c 'Access-Control' app/api/pitch-deck/upload/route.ts
   # Should be 0 (removed)
   ```
...
**Explicit per-file migration steps:**

1. `app/api/diagnostic/route.ts` -- Find `const rateLimitResult = checkRateLimit(...)` → add `await`
2. `app/api/diagnostic/positioning/route.ts` -- Find `const rateLimitResult = checkRateLimit(...)` → add `await`
3. `app/api/diagnostic/investor/route.ts` -- Find `const rateLimitResult = checkRateLimit(...)` → add `await`
4. `app/api/onboard/route.ts` -- Find `const rateLimitResult = checkRateLimit(...)` → add `await`
5. `app/api/auth/login/route.ts` -- Find `const rateLimitResult = checkRateLimit(...)` → add `await`
6. `app/api/admin/login/route.ts` -- Find `const rateLimitResult = checkRateLimit(...)` → add `await`
7. `app/api/fred/reality-lens/route.ts` -- Find `checkRateLimitForUser(...)` call → add `await`
8. `app/api/fred/memory/route.ts` -- Find `checkRateLimitForUser(...)` call → add `await`
9. `app/api/fred/chat/route.ts` -- Find `checkRateLimitForUser(...)` call → add `await`
10. `app/api/fred/history/route.ts` -- Find `checkRateLimitForUser(...)` call → add `await`
11. `app/api/fred/analyze/route.ts` -- Find `checkRateLimitForUser(...)` call → add `await`
12. `app/api/fred/decide/route.ts` -- Find `checkRateLimitForUser(...)` call → add `await`

For each file: read the file, find the exact line with `checkRateLimit` or `checkRateLimitForUser`, and prefix the call with `await`. All route handlers are already `async` so this is safe. If a caller destructures the result on the same line (e.g., `const { success } = checkRateLimit(...)`), change to `const { success } = await checkRateLimit(...)`.
