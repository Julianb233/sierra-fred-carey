...
### Non-Blocking Issues - All Resolved (2026-02-06)

| # | Issue | Status | Resolution |
|---|-------|--------|------------|
| 1 | Build trace fails (webpack mode) | DOCUMENTED | Next.js 16 webpack bugs: auto-migrates middleware.ts->proxy.ts, pages-manifest.json missing for App Router, build-trace NFT. Compilation + TS + tests all pass. Does not affect Vercel deployment. Build script updated to `--webpack`. |
| 2 | /api/onboard/invite lacks IP rate limiting | ALREADY FIXED | Rate limiting was already implemented (10 req/min IP-based via checkRateLimit) |
| 3 | 4 admin training endpoints return 503 | ALREADY REMOVED | Endpoints were deleted in a prior phase |
| 4 | Orphaned /dashboard/investor-evaluation page | REMOVED | Page and directory deleted |
| 5 | Orphaned investor-readiness-card.tsx component | REMOVED | Component deleted |
| 6 | 32 legacy DB tables not used by v1.0 | DOCUMENTED | See .planning/docs/database-schema-inventory.md (43 active, 11 legacy) |
| 7 | Duplicate table definitions across migrations | DOCUMENTED | See .planning/docs/migration-duplicates.md (7 duplicate tables, 4 conflicting numbers) |
