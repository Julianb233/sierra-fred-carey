...
**4. Dashboard stats are hardcoded mocks** — `app/dashboard/page.tsx` lines 118-202. Static values ("3", "1", "2", "4") and fake activity items.

**5. Dual auth import pattern** — `@/lib/auth` vs `@/lib/supabase/auth-helpers`. Both resolve to the same function, no functional impact.
