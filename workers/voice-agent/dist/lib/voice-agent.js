 ...
 */
export async function generateAgentToken(config) {
    if (!apiKey || !apiSecret) {
 ...
 */
export function getAgentSession(roomName) {
    return activeSessions.get(roomName) || null;
}
/**
 * Update agent session status
 */
export function updateAgentStatus(roomName, status) {
 ...
 */
export async function removeAgent(roomName) {
    try {
        const session = activeSessions.get(roomName);
        if (!session) {
            return false;
        ...
        if (apiKey && apiSecret && livekitUrl) {
            const client = new RoomServiceClient(livekitUrl.replace('wss://', 'https://'), apiKey, apiSecret);
 ...
 */
export async function fetchKnowledgeBase(configId) {
    const now = Date.now();
 ...
 */
export async function fetchEscalationRules(configId) {
    const now = Date.now();
 ...
 */
export function isAgentAvailable(roomName) {
    const session = activeSessions.get(roomName);
 ...
 */
export function getAgentConnectionDetails(roomName) {
    const session = activeSessions.get(roomName);
    if (!session) {
        return null;
    }
